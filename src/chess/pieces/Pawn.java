...
public class Pawn extends ChessPiece {
    ...
    public boolean[][] possibleMoves() {
        ...
        if (getColor ()==Color.WHITE) {
            ...
            if (getBoard ().positionExists ( p ) && (!getBoard ().thereIsAPiece ( p )) || isThereOpponetPiece ( p )){
                mat[p.getRow ()][p.getCollumn ()] = true;
            }
            p.setValues ( position.getRow () -2, position.getCollumn () );
            Position p2 = new Position ( position.getRow ()-1, position.getCollumn () );
            if (getBoard ().positionExists ( p ) && !getBoard ().thereIsAPiece ( p ) && getMoveCount () ==  0 && getBoard ().positionExists ( p2 ) && (!getBoard ().thereIsAPiece ( p2 ))||isThereOpponetPiece ( p )){
                mat[p.getRow ()][p.getCollumn ()] = true;
            }
